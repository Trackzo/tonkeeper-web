...
export const walletContractFromState = (wallet: WalletState, network?: Network) => {
    ...
    return walletContract(publicKey, wallet.active.version, network);
};

const workchain = 0;

export const walletContract = (publicKey: Buffer, version: WalletVersion, network?: Network) => {
    switch (version) {
        ...
        case WalletVersion.V5:
            return WalletContractV5.create({
                walletId: { workChain: workchain, networkGlobalId: network },
                publicKey
            });
    }
};
